...
public interface TeacherDAO {
	...
	public static boolean addgroup(String groupname,String groupdesc, 
			String selectedstudent) throws SQLException, ClassNotFoundException {
		Connection con = null;
		...
		con = CommonDAO.getConnection();
		pstmt = con.prepareStatement("insert into group_mst"
				+ "(groupname,groupdesc) values(?,?)");
		pstmt.setString(1, groupname);
		pstmt.setString(2, groupdesc);
		int recordCount = pstmt.executeUpdate();
		if(recordCount>0) {
			pstmtteachermap = con.prepareStatement("insert into "
					+ "group_teacher_mapping(groupid,teacherid) "
					+ "values((select groupid from group_mst "
					+ "where groupname=?),?)");
		...
		if(recordCount>0) {
		    ...
		    {
				pstmtmap = con.prepareStatement("insert into "
						+ "group_student_mapping(groupid,studentid) "
						+ "values((select groupid from group_mst where "
						+ "groupname=?),(select uid from user_mst where "
						+ "userid=?))");
	...
	public static boolean addtest(String testname,String marksperquestion,
			String selectedbranch,String selectedsemester,
			String selectedsubject,String selectedgroup) throws SQLException, ClassNotFoundException {
		Connection con = null;
		...
		if(recordCount>0) {
			pstmtteachermap = con.prepareStatement("insert into "
					...
					+ "values(?,(select testid from test_mst where "
					+ "testname=?))");
		...
		if(recordCount>0) {
			for(int i=0;i<groups.length;i++) {
				pstmt3 = con.prepareStatement("insert into "
						+ "test_branch_semester_subject_group_mapping"
						+ "(testid,branchid,semesterid,subjectid,"
						+ "groupid) values((select testid from test_mst "
						+ "where testname=?),(select branchid from "
						+ "branch_mst where branchname=?),(select semid "
						+ "from semester_mst where semestername=?),"
						+ "(select sid from subject_mst where "
						+ "subjectname=?),(select groupid from group_mst "
						+ "where groupname=?))");
		...
		System.out.println(selectedbranch+ selectedsemester+
				selectedsubject+selectedgroup);
		pstmt2 = con.prepareStatement("select testid from test_mst "
				+ "where testname=?");
		...
		pstmt4 = con.prepareStatement("select sid from "
				+ "subject_mst where subjectname=?");
	...
	public static boolean addquestion(String question,String choice1,
...
//		String regex=",";
		...
		pstmt1 = con.prepareStatement("insert into question_mst"
				+ "(questionname,choice1,choice2,choice3,choice4,"
		...
		if(recordCount>0) {
			pstmt2 = con.prepareStatement("insert into "
					+ "test_question_mapping(testid,questionid) "
					+ "values(?,(select questionid from "
					+ "question_mst where questionname=?))");
			...
			if(recordCount2>0) {
				pstmt3 = con.prepareStatement("insert into "
						+ "question_subject_mapping(questionid,"
						+ "subjectid) values((select questionid from "
						+ "question_mst where questionname=?),?)");
